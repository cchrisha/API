...
//             .populate('requestedBy', 'name email contact profession location') // Populate the requesting user's details
//             .sort({ createdAt: -1 });
...
//}
    ...
    app.get('/api/verification/notifications', verifyToken, async (req, res) => {
        try {
            ...
            // Fetch all verification notifications for the admin
            const notifications = await VerificationNotification.find({ requestedBy: user._id })
